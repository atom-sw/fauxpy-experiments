...
bar_scale <- function(data, mapping, ...)
{
  ...
  if (hasArg("boundary_mult")) {
    ...
    boundary_mult <- if (is.numeric(extra.params$boundary_mult)) extra.params$boundary_mult else 10
    ...
    potential_breaks <- round(seq(lower_limits[1], 
                                  ...
                                  lower_limits[2] + (lower_limits[2] - lower_limits[2])%%boundary_mult,
                                  by=boundary_mult))
    # Select a subset of nbins elements
    idx_breaks <- round(seq(from=1, to=length(potential_breaks), length.out=nbins))
