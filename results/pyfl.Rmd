...
```{r scatterplot}
p <- ggplot(joint.data, 
            aes(x=e_inspect.SBFL, y=e_inspect.MBFL, color=category)) + geom_point(size=3) + scale_color_met_d(C.PALETTE) + theme_classic()
...
data <- (datas 
         ...
         %>% filter(family == c("sbfl", "mbfl")))
data <- (data 
         %>% group_by(family, technique, category, project_name, bug_number) 
         %>% summarize(e_inspect=mean(e_inspect)))
...
p <- ggplot(joint.data, aes(x=e_inspect.SBFL, y=e_inspect.MBFL, color=category)) + geom_point(size=2) + geom_abline(slope=1, intercept=0) + facet_grid(cols=vars(technique.SBFL), rows=vars(technique.MBFL)) + scale_color_met_d(P.PALETTE)
p
```
