                          ...
                          info=NULL)
{
  ...
  res <- sapply(c(cor_txt, sig_txt, delta_txt), function(s) str_c(s, "\n"))
  if (!is.null(info)) {
...
points_abline <- function(data, mapping)
{
  ...
  pp <- (pp 
         + scale_x_continuous(limits=statement.einspect.limits) 
         + scale_y_continuous(limits=statement.einspect.limits))
...
ggsave(f.p.einspect, 
       plot=p.einspect, 
       device="pdf")
knitr::plot_crop(f.p.einspect, quiet=FALSE)
