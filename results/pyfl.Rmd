...
```{r}
eq.m1 <- brmsformula(
  mvbind(e_inspect, time) ~ 0 + bug + technique,
  family=brmsfamily("gaussian", link="log")
) + set_rescor(TRUE)

pp1.check <- get_prior(eq.m1, data=statement)
pp1 <- pp1.check
pp1$prior[pp1.check$class == "b"] <- "normal(4, 1.0)"
pp1$prior[grep("^(bug)|(technique)", pp1.check$coef)] <- "normal(0, 0.5)"
...
```{r m1 diagnostics}
(prior.check1 <- pp_check(m1.priors, draw_ids=1:100, resp="time") + scale_x_log10())
```
