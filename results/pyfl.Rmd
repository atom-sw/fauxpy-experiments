...
interval.results.interacts <- function(fit, response, predictor, probs, bool=TRUE)
{
  ps.low <- rev(1-probs)/2
  ps.high <- rev(1-ps.low)
  ps <- rev(c(ps.high, ps.low))
...
(res.m4.crashing <- interval.results.interacts(m4, "einspectS", 
                                              "crashing", PROBABILITIES))
(res.m4.predicate <- interval.results.interacts(m4, "einspectS", 
                                              "predicate", PROBABILITIES))
(res.m4.mutable <- interval.results.interacts(m4, "einspectS", 
...
Alternative ways would include:
...
in terms of predictive capabilities.
However, they both mop the floor with all other models (14 standard deviations of difference!). Note, however, that this is largely a result of
