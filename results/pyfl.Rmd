...
$m_1$'s score is more than 2.9 standard deviations worse than $m_2$'s,
which is a pretty significant difference in favor of $m_2$
...
$m_2$'s score is less than half standard deviation worse than $m_3$'s.
This is a negligible improvement, not worth 
the additional complexity of model $m_3$.
...
thus we specify two different formulas for different outcomes.
...
eq.m4.einspect <- brmsformula(einspectS ~ 1 
                              ...
                              + mutable*family,
                              family=brmsfamily("gaussian", link="log"))
eq.m4.time <- brmsformula(timeS ~ 1 + (1|p|family) + (1|q|category),
                          family=brmsfamily("gaussian", link="log"))

eq.m4 <- eq.m4.einspect + eq.m4.time + set_rescor(TRUE)

pp4.check <- get_prior(eq.m4, data=by.statement)

pp4 <- c(
  set_prior("normal(0, 1.0)", class="Intercept", resp=c("einspectS", "timeS")),
  set_prior("normal(0, 1.0)", class="b", resp=c("einspectS")),
  set_prior("weibull(2, 0.3)", class="sd", coef="Intercept", 
            group="family", resp=c("einspectS", "timeS")),
  set_prior("weibull(2, 0.3)", class="sd", coef="Intercept", 
            group="category", resp=c("einspectS", "timeS")),
...
Around 2.4 standard deviations of improvement is quite good,
which implies that the bug-kind effects can capture
...
eq.m5 <- eq.m5.einspect + eq.m4.time + set_rescor(TRUE)
...
eq.m6 <- eq.m6.einspect + eq.m4.time + set_rescor(TRUE)
...
in terms of predictive capabilities.
And, they both overperform all other models.
Thus, we'll use $m_6$ for all our analysis.
...
summary(m6)
```
Let's now analyze $m_7$ first, whose mutability-related effects
are fixed effects.

```{r m6 analysis}
MORE.PROBS <- c(0.5, 0.7, 0.8, 0.87, 0.9, 0.95, 0.99)
...
but it can only be detected with 87%
