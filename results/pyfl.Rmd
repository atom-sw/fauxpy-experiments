...
interval.results <- function(fit, response, group, predictor="technique")
{
  # posterior draws
  ...
  colnames(diffs) <- TECHNIQUES
  # reshape
  diffs <- data.frame(diffs) %>% gather(predictor, response)
  # add family column
  diffs$family <- sapply(diffs$predictor, which.family)
  # compute HPDIs
  intervals <- (diffs
                %>% group_by(.data[[group]]) 
...
res.m1.einspect <- interval.results(m1, "einspectS", "family")
res.m1.time <- interval.results(m1, "timeS", "family")
```
