...
bar_scale <- function(data, mapping, ...)
{
  ...
  # Number of bins, with default
  nbins <- if (hasArg("bins")) extra.params$bins else 20
  # Number of bins with displayed label, with default
  nlabels <- if (hasArg("n_labels")) extra.params$n_labels else 5
  # Boundary values between bins
  ...
  idx_labels <- round(seq(from=1, to=length(labels), length.out=nlabels))
  ...
  pp <- pp + scale_x_discrete(drop=FALSE, labels=text_labels)
